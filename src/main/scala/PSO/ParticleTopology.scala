...
object ParticleTopology extends Enumeration {
  ...
  def connectParticlePair(a: ActorRef, b: ActorRef) = {
    a ! NewNeighbour(b)
    b ! NewNeighbour(a)
  }
  
  def formRing(particles: List[ActorRef]) = {
    connectParticlePair(particles.head, particles.takeRight(1).head)
    formChain(particles)
  }

  def formChain(particles: List[ActorRef]): Unit = {
    if (particles.size > 1) {
  ...
  def fullyConnected(particles: Set[ActorRef]) = {
    particles.foreach(p => p ! UpdateNeighbourhood(particles.diff(Set(p))))
  }

  def buildGrid(particles: List[ActorRef]):
  ...
  def connectGrid(width: Int, positions: List[(Int,Int)],
  ...
  def formGrid(particles: List[ActorRef]) = {
    val (width, pos, grid) = buildGrid(particles)
    connectGrid(width, pos, grid)
  }

  def connectToroidal(width: Int, grid: Array[Array[ActorRef]]) = {
  ...
  def formToroidal(particles: List[ActorRef]) = {
    val (width, pos, grid) = buildGrid(particles)
    connectGrid(width, pos, grid)
    connectToroidal(width, grid)
  }

  def formStar(particles: List[ActorRef]) = {
    particles.tail.foreach(p => connectParticlePair(p, particles.head))
  }

  def formBinaryTree(particles: List[ActorRef]) = {
